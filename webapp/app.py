...
def build_override_config(job_dir, form):
    ...
    elif form["ligand_prep_tool"] == "pypolybuilder":
        payload.update(
            {
                "PYPOLYBUILDER_BIN": form.get("pypolybuilder_bin", "pypolybuilder"),
                "PYPOLYBUILDER_MODE": form["pypolybuilder_mode"],
                "PYPOLYBUILDER_PARAMS": form["pypolybuilder_params"],
                "PYPOLYBUILDER_FORCEFIELD_PATH": form.get("pypolybuilder_forcefield_path", ""),
                "PYPOLYBUILDER_CORE": form.get("pypolybuilder_core", ""),
                "PYPOLYBUILDER_TERMINAL": form.get("pypolybuilder_terminal", ""),
                "PYPOLYBUILDER_INTERMEDIATE": form.get("pypolybuilder_intermediate", ""),
                "PYPOLYBUILDER_BBS": form.get("pypolybuilder_bbs", ""),
                "PYPOLYBUILDER_CONNECTIONS": form.get("pypolybuilder_connections", ""),
                "PYPOLYBUILDER_NGEN": form.get("pypolybuilder_ngen", "0"),
                "PYPOLYBUILDER_NSTEPS": form.get("pypolybuilder_nsteps", "200"),
                "PYPOLYBUILDER_NGENGA": form.get("pypolybuilder_ngenga", "20"),
                "PYPOLYBUILDER_NPOP": form.get("pypolybuilder_npop", "25"),
                "PYPOLYBUILDER_EXTRA_ARGS": form.get("pypolybuilder_extra_args", ""),
            }
        )
...
def write_umbrella_inputs_required_summary(job_dir, source_dir):
    output_dir = job_dir / "analysis" / "umbrella_sampling"
    output_dir.mkdir(parents=True, exist_ok=True)
    lines = [
        "GROMACS WHAM Umbrella Sampling Analysis",
        "Status: waiting for umbrella-window inputs",
        f"Generated: {datetime.now().isoformat(timespec='seconds')}",
        f"Source checked: {source_dir}",
        "",
        "This linked task cannot run from ordinary production MD files alone.",
        "Upload independent umbrella-window umbrella*.tpr files plus matching unique umbrella*_pullf.xvg files in the Umbrella tab.",
        "If pullf files are absent, pullx files are accepted. The app creates tpr-files.dat plus pullf-files.dat or pullx-files.dat and runs gmx wham -unit kCal.",
        "Reference: https://manual.gromacs.org/current/onlinehelp/gmx-wham.html",
    ]
    (output_dir / "analysis_summary.txt").write_text("\n".join(lines) + "\n", encoding="utf-8", newline="\n")


def run_requested_linked_analyses(job_dir, analysis_plan, source_dir):
    if analysis_plan.get("free_energy") == "yes":
        write_linked_analysis_summary(job_dir, "free_energy", "Linked Free Energy Analysis", source_dir)
    if analysis_plan.get("umbrella_sampling") == "yes":
        write_umbrella_inputs_required_summary(job_dir, source_dir)
...
def write_umbrella_delta_g(profile_xvg, summary_path):
    x_vals, y_vals = read_xvg_xy(profile_xvg)
    if not y_vals:
        return
    min_value = min(y_vals)
    max_value = max(y_vals)
    delta_g = max_value - min_value
    min_x = x_vals[y_vals.index(min_value)] if x_vals else "n/a"
    max_x = x_vals[y_vals.index(max_value)] if x_vals else "n/a"
    with Path(summary_path).open("a", encoding="utf-8", newline="\n") as handle:
        handle.write("\nPMF result from attachment rule:\n")
        handle.write("Delta G is highest PMF minus lowest PMF, assuming PMF converges at large COM distance.\n")
        handle.write(f"Lowest PMF: {min_value:.6g} kcal mol^-1 at xi={min_x}\n")
        handle.write(f"Highest PMF: {max_value:.6g} kcal mol^-1 at xi={max_x}\n")
        handle.write(f"Delta G: {delta_g:.6g} kcal mol^-1\n")
...
def run_umbrella_wham_analysis(job_dir, analysis_dir, form):
    output_dir = Path(analysis_dir) / "umbrella_sampling"
    output_dir.mkdir(parents=True, exist_ok=True)
    summary_path = output_dir / "analysis_summary.txt"
    summary_lines = [
        "GROMACS WHAM Umbrella Sampling Analysis",
        "Source: Umbrella Sampling.pdf",
        f"Generated: {datetime.now().isoformat(timespec='seconds')}",
        "",
        "Attachment workflow:",
        "1. Use independent umbrella windows generated from selected pulling configurations.",
        "2. Prepare tpr-files.dat with umbrella*.tpr files.",
        "3. Prepare pullf-files.dat or pullx-files.dat with matching unique pull files.",
        "4. Run: gmx wham -it tpr-files.dat -if pullf-files.dat -o -hist -unit kCal",
        "",
    ]
    summary_path.write_text("\n".join(summary_lines), encoding="utf-8", newline="\n")

    try:
        unpack_analysis_archives(analysis_dir)
    except ValueError as exc:
    ...
    if not tpr_files or not selected_pull_files:
        with summary_path.open("a", encoding="utf-8", newline="\n") as handle:
            handle.write("\nStatus: waiting for umbrella-window inputs\n")
            handle.write("Upload umbrella*.tpr files and matching umbrella*_pullf.xvg files. pullx files are accepted only when pullf files are absent.\n")
        return
    ...
    write_umbrella_delta_g(profile_xvg, summary_path)
    run_optional_plot(profile_xvg, "Umbrella PMF (kcal mol^-1)", output_dir / "profile.png")
    run_optional_plot(histo_xvg, "Umbrella Histograms", output_dir / "histo.png")
    with summary_path.open("a", encoding="utf-8", newline="\n") as handle:
        handle.write("\nStatus: completed\n")
        handle.write("Check histogram overlap. If there is a poorly sampled gap, add another window in that region and rerun WHAM.\n")
    write_done_stamps(job_dir, ["umbrella_sampling"])


def save_pypolybuilder_uploads(job_dir, mode):
    pypoly_dir = job_dir / "inputs" / "pypolybuilder"
    saved = {
        "pypolybuilder_params": save_optional_upload(request.files.get("pypolybuilder_params"), pypoly_dir / "params.itp"),
        "pypolybuilder_core": "",
        "pypolybuilder_terminal": "",
        "pypolybuilder_intermediate": "",
        "pypolybuilder_bbs": "",
        "pypolybuilder_connections": "",
    }
    if mode == "dendrimer":
        saved["pypolybuilder_core"] = save_optional_upload(request.files.get("pypolybuilder_core"), pypoly_dir / "core.itp")
        saved["pypolybuilder_terminal"] = save_optional_upload(request.files.get("pypolybuilder_terminal"), pypoly_dir / "terminal.itp")
        saved["pypolybuilder_intermediate"] = save_optional_upload(request.files.get("pypolybuilder_intermediate"), pypoly_dir / "intermediate.itp")
        missing = [label for key, label in (("pypolybuilder_params", "parameter .itp"), ("pypolybuilder_core", "core .itp"), ("pypolybuilder_terminal", "terminal .itp")) if not saved[key]]
    else:
        bbs_paths = []
        for index, upload in enumerate(request.files.getlist("pypolybuilder_bbs"), start=1):
            if upload and upload.filename:
                destination = pypoly_dir / f"building_block_{index}.itp"
                save_upload(upload, destination)
                bbs_paths.append(str(destination))
        saved["pypolybuilder_bbs"] = ",".join(bbs_paths)
        saved["pypolybuilder_connections"] = save_optional_upload(request.files.get("pypolybuilder_connections"), pypoly_dir / "connections.in")
        missing = [label for key, label in (("pypolybuilder_params", "parameter .itp"), ("pypolybuilder_bbs", "building-block .itp"), ("pypolybuilder_connections", "connections file")) if not saved[key]]
    if missing:
        raise ValueError("pyPolyBuilder requires " + ", ".join(missing) + ".")
    return saved
...
def full_auto_job():
    ...
    if form["ligand_prep_tool"] != "acpype":
        flash("Full Automation supports ACPYPE ligand preparation only.", "error")
        return redirect(url_for("index"))
...
def special_analysis_job():
    ...
    elif analysis_type == "umbrella_sampling":
        disabled_dir = analysis_dir / "umbrella_sampling"
        disabled_dir.mkdir(parents=True, exist_ok=True)
        (disabled_dir / "analysis_summary.txt").write_text("Umbrella Sampling Analysis\nStatus: disabled in this app version\n", encoding="utf-8", newline="\n")
...
def topology_upload(job_id):
    ...
    try:
        ...
        pypolybuilder_ngen = normalize_nonnegative_int(request.form.get("pypolybuilder_ngen"), "pyPolyBuilder generations", required=False)
        pypolybuilder_nsteps = normalize_positive_int(request.form.get("pypolybuilder_nsteps", "200"), "pyPolyBuilder geometry steps")
        pypolybuilder_ngenga = normalize_positive_int(request.form.get("pypolybuilder_ngenga", "20"), "pyPolyBuilder GA generations")
        pypolybuilder_npop = normalize_positive_int(request.form.get("pypolybuilder_npop", "25"), "pyPolyBuilder GA population")
    except ValueError as exc:
        flash(str(exc), "error")
        return redirect(url_for("topology_upload", job_id=job_id))
    ...
    elif ligand_prep_tool == "pypolybuilder":
        try:
            pypolybuilder_files = save_pypolybuilder_uploads(job_dir, request.form.get("pypolybuilder_mode", "dendrimer"))
        except ValueError as exc:
            flash(str(exc), "error")
            return redirect(url_for("topology_upload", job_id=job_id))
    elif ligand_prep_tool != "acpype":
    ...
    form = {
        ...
        "pypolybuilder_bin": request.form.get("pypolybuilder_bin", "pypolybuilder"),
        "pypolybuilder_mode": request.form.get("pypolybuilder_mode", "dendrimer"),
        "pypolybuilder_forcefield_path": request.form.get("pypolybuilder_forcefield_path", ""),
        "pypolybuilder_ngen": pypolybuilder_ngen or "0",
        "pypolybuilder_nsteps": pypolybuilder_nsteps,
        "pypolybuilder_ngenga": pypolybuilder_ngenga,
        "pypolybuilder_npop": pypolybuilder_npop,
        "pypolybuilder_extra_args": request.form.get("pypolybuilder_extra_args", ""),
